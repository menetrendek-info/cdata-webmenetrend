...
export function NavbarMinimal({ data, doBreak }: { data: Array<NavbarLinkProps>, doBreak: boolean }) {
    ...
    return (<>
        <Navbar p={!doBreak ? "md" : undefined} sx={{
            position: "fixed", display: "flex",
            "@media (max-width: 600px)": { bottom: 0, alignItems: "center", justifyContent: "center", },
        }} height={doBreak ? navSize : "100vh"} width={{ base: !doBreak ? navSize : "100vw" }}>
            <Stack>
                {doBreak ? <></> : <Center>
                    <Logo size={45} />
                </Center>}
